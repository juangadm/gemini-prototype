...
export default function ValueToast({
  isVisible,
  onDismiss,
  tier,
  extraGenerations,
  ...
  return (
    <div
      className="fixed z-50 animate-toast-enter"
      style={{
        bottom: '100px',
        right: '24px',
      }}
    >
      <div
        className="bg-white rounded-2xl border border-[#e8eaed] p-4 flex items-start gap-3 max-w-[320px]"
      ...
      >
        ...
        <button
          onClick={onDismiss}
          className="flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-full hover:bg-[#f1f3f4] transition-colors -mt-1 -mr-1"
          aria-label="Dismiss"
        >
          <span className="material-symbols-outlined text-[#5f6368] text-lg">close</span>
        </button>
      </div>
    </div>
  )
